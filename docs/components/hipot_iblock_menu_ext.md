...
Компонент реализован классом `Hipot\Components\IblockMenuExt` в `class.php`.
...
Компонент использует data-кеш `Bitrix\Main\Data\Cache` без кеширования вывода. Идентификатор строится из всех `$arParams`, поэтому разные инфоблоки, фильтры, сортировки, модификаторы и версии получают разные записи.
