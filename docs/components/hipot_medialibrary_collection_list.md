...
Компонент реализован классом `Hipot\Components\MedialibraryCollectionList` в `class.php`.
...
У компонента нет `.parameters.php`, поэтому параметры вызова задаются в PHP-коде. Если дочерний компонент не вернул ключ `ITEMS`, компонент считает список элементов пустым.
