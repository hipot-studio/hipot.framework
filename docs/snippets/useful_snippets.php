...
<?
// дл€ производительности и прочие фиксы на крон
...
<?
// old-way ¬ыбор элементов со свойствами
...
while ($arItem = $rsItems->GetNext()) {
	...
	while ($ar_props = $db_props->GetNext()) {
		// дл€ свойств TEXT/HTML не верно экранируютс€ символы
...
<?
// ѕример выборки дерева подразделов дл€ раздела
		...
		$(__np).click(function(){
			...
			$.ajax({
				...
				success: function(data, textStatus, jqXHR){
					$(_npp).html('¬ам на email прийдет сообщение о поступлении товара на сайт.').css('top', '-20px').css('cursor', 'default');
				}
			});
...
if (count($arResult["SEARCH"]) > 0) {
	...
	foreach ($arIDs as $iblockId => $searchIds) {
		// дл€ инфоблоков 2.0 передавать IBLOCK_ID дл€ выбора свойств об€зательно
