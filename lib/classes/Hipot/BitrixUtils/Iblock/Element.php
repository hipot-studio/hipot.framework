...
use CFile, CIBlockElement, CIBlockProperty, CIBlockPropertyEnum;
...
trait Element
{
	 ...
	 */
	public static function addElementToDb(array $arAddFields = [], bool $bUpdateSearch = false): UpdateResult
	{
	 ...
	 */
	public static function updateElementToDb(int $ID, array $arAddFields = [], bool $bUpdateSearch = false): UpdateResult
	{
		if (! is_array($arAddFields)) {
			$arAddFields = [];
		}
	 ...
	 */
	public static function checkElementExistsByNameOrCode($fieldVal, int $iblockId, string $fieldType = 'name')
	{
	 ...
	 * @return false|int
	 */
	public static function checkExistsEnumByVal(string $propCode, int $iblockId, $val)
	{
		$val = trim((string)$val);
		if ($val == '') {
			return false;
		...
		if (is_null($existsValues)) {
			$vs = self::selectPropertyEnumArray($propCode, ['IBLOCK_ID' => $iblockId]);
			foreach ($vs as $v) {
				$existsValues[ trim($v['VALUE']) ] = $v['ID'];
			}
		}
		if (is_null($PROPERTY_ID)) {
			$arProp = CIBlockProperty::GetList([], ['IBLOCK_ID' => $iblockId, 'CODE' => $propCode])->Fetch();
	 ...
	 * @param int   $IBLOCK_ID
	 * @param int   $ID
	 * @param array{'ELEMENT_META_DESCRIPTION':string, 'ELEMENT_META_TITLE':string, 'ELEMENT_META_KEYWORDS':string} $arTemplates	 *
	 * @throws \Bitrix\Main\ArgumentException
	 */
	public static function setSeoValues(int $IBLOCK_ID, int $ID, array $arTemplates = []): void
	{
